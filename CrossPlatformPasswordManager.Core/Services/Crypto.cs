...
public static class Crypto
{
    ...
    /// </summary>
    /// <param name="password">The raw master password input from the user.</param>
    ...
    public static bool VerifyMasterPassword(string password, string passwordHash) =>
    ...
    /// <param name="base64Salt">The key derivation salt stored with the master password hash.</param>
    /// <returns>An AES encryption key to be used when encrypting and decrypting password entries.</returns>
    public static byte[] DeriveAesKey(string password, string base64Salt)
    {
        var saltBytes = GetBase64SaltBytes(base64Salt);
        var passwordBytes = Encoding.UTF8.GetBytes(password);
        var config = GenerateArgon2Config(passwordBytes, saltBytes);
        using var argon2 = new Argon2(config);
    ...
    /// </summary>
    /// <param name="size">The size, in bytes, of the salt to generate. The default value is 16.</param>
    /// <returns>A Base64-encoded string representing the generated salt.</returns>
    public static string GenerateBase64Salt(int size = 16) =>
    ...
    /// </summary>
    /// <param name="base64Salt">The Base64-encoded string to convert. Must be a valid Base64 string.</param>
    /// <returns>An array of bytes decoded from the specified Base64 string.</returns>
    public static byte[] GetBase64SaltBytes(string base64Salt) =>
    ...
    public static Argon2Config GenerateArgon2Config(byte[] passwordBytes, byte[] saltBytes) =>
        new()
        {
    ...
    /// ciphertext.</returns>
    public static string EncryptEntry(byte[] aesKey, string plaintext)
    {
        if (aesKey.Length != 32)
    ...
    public static string DecryptEntry(byte[] aesKey, string encryptedData)
    {
        if (aesKey.Length != 32)
            throw new ArgumentException("AES key must be 256 bits (32 bytes).", nameof(aesKey));
